...
class ValidadorRegistro {
	...
	private function validar_nombre($conexion, $nombre) {
		...
		if (strlen($nombre) <6) {
			return "El nombre tiene que tener más de 6 caracteres.";
		}
		if (strlen($nombre) >24) {
	...
	private function validar_email($conexion, $email) {
		if(!$this -> variable_inciada($email)) {
		...
		if (RepositorioUsuario :: email_existe ($conexion, $email)) {
			return "Este email ya está en uso. Introduce otro, o <a href=#>recupera tu contraseña</a>";
		}
	...
	private function validar_clave1($clave1) {
		if(!$this -> variable_inciada($clave1)) {
	...
	private function validar_clave2($clave1, $clave2) {
		...
		if(!$this -> variable_inciada($clave1)) {
			return "Primero introduce la contraseña";
		}

		if(!$this -> variable_inciada($clave2)) {
